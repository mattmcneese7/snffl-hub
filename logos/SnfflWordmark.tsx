...
const CSS = `
.snffl-wave{animation:snffl-wave 2.4s linear infinite}
.snffl-fall{transform-box:fill-box;transform-origin:50% 0;animation:snffl-drop 3.4s cubic-bezier(.5,0,.9,.6) infinite;transition:opacity .2s}
.snffl-compact .snffl-fall{opacity:0;animation-play-state:paused}
.snffl-tilt{transform-box:view-box;transform-origin:150px 58px}
.snffl-tilt.snffl-slosh{animation:snffl-slosh 1.7s cubic-bezier(.3,.6,.4,1)}
@keyframes snffl-wave{to{transform:translateX(-40px)}}
@keyframes snffl-drop{0%{transform:translateY(-4px) scale(.25);opacity:0}35%{transform:none;opacity:1}55%{transform:none;opacity:1}90%{transform:translateY(24px);opacity:0}100%{opacity:0}}
@keyframes snffl-slosh{15%{transform:rotate(-7deg) translateY(-6px)}35%{transform:rotate(5deg)}55%{transform:rotate(-3deg)}75%{transform:rotate(1.5deg)}100%{transform:none}}
@media (prefers-reduced-motion:reduce){.snffl-wave,.snffl-fall,.snffl-tilt.snffl-slosh{animation:none}}
`;

/**
 * SNFFL wordmark. Letters use currentColor, so set the text color
 * (ink #141210 in light, #F3EFE6 in dark) on the parent.
 */
